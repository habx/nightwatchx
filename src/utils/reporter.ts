...
export default async (globals: NightwatchGlobals, results) => {
  ...
  for (let moduleReportName of currentModules) {
    ...
    const failedTests = reduce(get(moduleReport, 'completed'), (ctx, completedTest, testName) => {
      if (completedTest.errors > 0 || completedTest.failed > 0) {
        return {
          ...ctx,
          [testName]: get(completedTest, 'lastError') || get(completedTest, 'stackTrace'),
        }
      }
      return ctx
    }, {})
    let failedScreenshotCompareCount = 0
    let firstScreenshotFailed = get(generalReport, [moduleReportName, 'firstScreenshotFailed'])
    ...
    if (testsCount > 0) {
      generalReport[moduleReportName] = {
        ...
        failedImageUrl: globals.failedImageUrl || get(generalReport, [moduleReportName, 'failedImageUrl']),
        failedTests: {
          ...get(generalReport, [moduleReportName, 'failedTests'], {}),
          ...failedTests,
        }, // TODO: add errors
        envs : uniq([...get(generalReport, [moduleReportName, 'envs'], []), globals.env]),
        devices : uniqBy([...get(generalReport, [moduleReportName, 'devices'], []), {
          label: globals.deviceName,
          uiRegression: failedScreenshotCompareCount > 0,
          failed: Object.values(failedTests).length > 0,
        }], 'label'),
        totalTime: Number(get(generalReport, [moduleReportName, 'totalTime'], 0)) + Number(get(moduleReport, 'time', 0)),
        testsCount,
        failedCount: Number(get(generalReport, [moduleReportName, 'failedCount'], 0)) + Number(get(moduleReport, 'failedCount', 0)),
        errorsCount: Number(get(generalReport, [moduleReportName, 'errorsCount'], 0)) + Number(get(moduleReport, 'errorsCount', 0)),
        passedCount: Number(get(generalReport, [moduleReportName, 'passedCount'], 0)) + Number(get(moduleReport, 'passedCount', 0)),
        firstScreenshotFailed,
        failedScreenshotCompareCount: failedScreenshotCompareCount + Number(get(generalReport, [moduleReportName, 'failedScreenshotCompareCount'], 0)),
      }
    }
  }
