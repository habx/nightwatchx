...
export const uploadFile = (file: Buffer, filePath: string, browser?: NightwatchBrowser) => new Promise((resolve, reject) => {
  const interval = preventIdleTimeout(browser, 'Uploading file...')
  s3.putObject({ Bucket: BUCKET_NAME, Key: filePath, Body: file, ContentDisposition: 'inline', ContentType: 'image/png' }, (err, data) => {
    if (err) {
      reject(err)
    } else resolve(data)
...
export const getFile = (filePath: string, browser?: NightwatchBrowser): Promise<Buffer> => new Promise((resolve) => {
...
export const getFileUrl = (filePath: string): string => s3.getSignedUrl('getObject', { Bucket: BUCKET_NAME, Key: filePath, Expires: 60 * 60 * 24 })
