    ...
    try {
      ...
      if ((testOnly.length === 0 || testOnly.includes(testSuiteName))) {
        ...
        for (const run of Object.keys(manifest.runs)) {
          ...
          const frequencyMinutes = (get(manifest, ['runs', run, 'frequencyMinutes']) || get(manifest, ['runs', 'default', 'frequencyMinutes'])) as number
          const shouldRun = localParam || (lastRunTme && frequencyMinutes ? runDiffMinutes > frequencyMinutes : true)

          if (shouldRun) {
            const getTestSuiteConfigPath = (envName: string, isTest = false) => `./dist/${testSuiteName}_${run}_${envName}${isTest ? '.local' : ''}.js`
            const devicesEnv = get(manifest, ['runs', run, 'devices'], ['default'])
            ...
            for (const envName of env) {
              if (localParam) {
                shell.exec(`./node_modules/.bin/nightwatch -c ${getTestSuiteConfigPath(envName, true)}`)
              } else {
                for (const deviceEnv of devicesEnv) {
                  shell.exec(`./node_modules/.bin/nightwatch -c ${getTestSuiteConfigPath(envName)} -e ${deviceEnv}`)
            ...
            console.log(`Skipping run ${run} for test suite ${testSuiteName} (last run was ${runDiffMinutes} minutes ago)`)
          }
