...
import { generateLocalConfig } from '../config/generateConfig'
  ...
  try {
    ...
    Object.keys(manifest.runs).forEach(run => {
      ...
      envs.forEach(env => {
        ...
        const testSuiteConfig = generateLocalConfig(testSuiteName, currentConfig, env)
        const testSuiteConfigPath = `./dist/${testSuiteName}_${run}_${env}.local.js`
